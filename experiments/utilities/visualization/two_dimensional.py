...
def get_plot_slice_data(
    tensor, axis=2, idx=20, plot_uvw="uv", color_scheme="hv"
):
    """Returns the data for the plot_slice call"""
    data = tensor.data.numpy()
    data = np.take(
        np.moveaxis(data[get_uvw_indices(plot_uvw)], 0, -1), idx, axis=axis
    )
...
def plot_slice(
    tensor, filename, axis=2, idx=20, plot_uvw="uv", color_scheme="hv"
):
...
def plot_slice_comparison(
    tensors, filename, axis=2, idx=20, plot_uvw="uv", color_scheme="hv"
):
