...
def test_GradientConstraint():
    ...
    metrics = {
        'dummy_constraint': GradientConstraint(
            constraint, output_transform=lambda args: (
                args[0], args[0], {'dummy_kwarg': 'dummy_value'})
        ),
    ...
    # Dummy data
    dummy_dl = DataLoader(TensorDataset(
        torch.tensor([[1, 1, 1]])), batch_size=1)
    ...
    assert(torch.allclose(
        engine.state.metrics['dummy_sum'], 2 * engine.state.metrics['dummy_constraint']))
